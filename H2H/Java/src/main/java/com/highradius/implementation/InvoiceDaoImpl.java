...
public class InvoiceDaoImpl{
	...
	public int insertInvoice(int customerOrderId,int salesOrg, String distributionChannel,int customerNumber,int companyCode, String orderCurrency,Double amountInUSD,String orderCreationDate) {
		 ...
		 try (Connection connection = DatabaseConnection.connect()) {
	         ...
	         // Execute a query
	         PreparedStatement ps = connection.prepareStatement("insert into h2h_oap(customer_Order_Id,SALES_Osys_configRG, distribution_Channel,company_Code,order_Creation_Date, order_Currency,customer_Number,amount_In_USD) values(?,?,?,?,?,?,?,?) ");
	...
	public ArrayList<Invoice> getInvoice(){
		...
		try (Connection connection = DatabaseConnection.connect()) {
	         ...
	         // Execute a query
	         String query = "SELECT sl_no,customer_Order_Id,SALES_Osys_configRG, distribution_Channel,company_Code,order_Creation_Date, order_Currency,customer_Number,amount_In_USD,order_Amount FROM h2h_oap LIMIT 1000";
